...
result_t ld_resolve_load_symbol(const char *find_name, Elf64_Sym **def, module_t **defining_module) {
	...
	trn_list_foreach(&ld_module_list_head, i) {
		result_t r = ld_try_resolve_symbol(trn_list_entry(module_list_node_t, list, i)->module, find_name, find_name_hash, def, defining_module);
...
result_t ld_resolve_dependency_symbol(module_t *find_mod, const char *find_name, Elf64_Sym **def, module_t **defining_module) {
	uint64_t find_name_hash = elf_hash_string(find_name);
	result_t r;
	r = ld_try_resolve_symbol(find_mod, find_name, find_name_hash, def, defining_module);
	if(r != LIBTRANSISTOR_ERR_TRNLD_COULD_NOT_RESOLVE_SYMBOL) { // this includes RESULT_OK
		return r;
	}
	trn_list_foreach(&find_mod->dependencies, i) {
		r = ld_try_resolve_symbol(trn_list_entry(module_list_node_t, list, i)->module, find_name, find_name_hash, def, defining_module);
		if(r == LIBTRANSISTOR_ERR_TRNLD_COULD_NOT_RESOLVE_SYMBOL) {
			continue;
		} else { // this includes RESULT_OK
			return r;
