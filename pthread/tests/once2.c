...
myfunc(void)
{
  ...
  phal_mutex_lock(numOnce.cs);

  numOnce.i++;

  phal_mutex_unlock(numOnce.cs);
...
mythread(void * arg)
{
  ...
  phal_mutex_lock(numThreads.cs);
  numThreads.i++;
  phal_mutex_unlock(numThreads.cs);
...
int pthread_test_once2()
{
  ...
  phal_mutex_create(&numThreads.cs);
  phal_mutex_create(&numOnce.cs);
  ...
  phal_mutex_destroy(numOnce.cs);
  phal_mutex_destroy(numThreads.cs);
